...
int main(int argc, char *argv[])
{
  ...
  while (1)
  {
    ...
    ssize_t len     = recvfrom(tRawUDPSock,
                               abPktInfo,
                               sizeof(abPktInfo),
                               0 /*flags*/,
    ...
    printf(
      "%s recv from peer: %s, len: %zd with IPPROTO_IPV6 setting: %d . src port: %u, dst port: %u;  no ip header included src port: %u, dst port: %u\n",
      szFmtTimeBuf,
      szSrcAddrBuf,
      len,
      no,
      ntohs(ptUDPHdr->u16SrcPort),
      ntohs(ptUDPHdr->u16DstPort),
      ntohs(ptUDPHdr_NO_IPHDRINCL->u16SrcPort),
      ntohs(ptUDPHdr_NO_IPHDRINCL->u16DstPort));
