...
#define RC_EXCEPTION_RETURN(EXPR, FORMAT, args ...) \
        do { \
          if (EXPR) \
          { \
...
#define RC_EXCEPTION_GOTO_FAIL(EXPR, FORMAT, args ...) \
        do { \
          if (EXPR) \
          { \
            strerror_r(errno, szErrno, sizeof(szErrno)); \
...
int __attribute__((nonnull(2))) nl_filter_route(int family,
                                                const struct nlmsghdr *n,
                                                unsigned short expected_msgtype,
                                                void                  *arg)
{
  printf("recv para: %p\n", arg);
...
int main(int argc, char *argv[])
{
  ...
  ssize_t recvlen = recvfrom(tNetlinkUDPSock,
                             buf,
                             sizeof(buf),
  ...
  {
    if ((h->nlmsg_pid == 0) ||
        ((h->nlmsg_pid != 0) && ((h->nlmsg_pid != req.nlh.nlmsg_pid) && ((pid_t)h->nlmsg_pid != self_pid))))
    {
      fprintf(stderr,
              "error: not expected response msg! self_pid: %d, request nlmsg_pid: %d, response nlmsg_pid: %d",
              self_pid,
              req.nlh.nlmsg_pid,
              h->nlmsg_pid);
      goto skip_it;
    ...
    if (h->nlmsg_type == NLMSG_ERROR)
    {
      ...
      else
      {
        ...
        if (err->error == 0)
        {
          ...
          printf(
            "got response ack! self_pid: %d, request nlmsg_pid: %d, seq: %d, response nlmsg_pid: %d, seq: %d",
            self_pid,
            req.nlh.nlmsg_pid,
            req.nlh.nlmsg_seq,
            h->nlmsg_pid,
