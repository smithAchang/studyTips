...
# 前言
本来想把`C`语言利用宏进行元编程的技巧经验，总结到《C语言编程规范》中，但是体量太大了，还是单独成文档能够说得清楚 :)
...
在`C`语言的层面，用宏可以在编译期进行生产代码，所以，可以说宏是`C`语言元编程的工具！
...
在`C`语言中，如果代码中存在模式，则可以定义出来全信息的宏模板，宏模板体现为多行宏定义。

## 宏模板遍历器
经常使用无名临时宏，作为宏模板的遍历器，临时无名宏，可以看为不断变化的宏变量，可以对宏模板进行不同切面的展开。

遍历器根据宏模板的指导，可以生成联合`union`、联合成员、结构体`struct` 结构体成员、全局变量、全局变量初始化列表、函数声明和定义`function declaration and definition`等语言基本构件。
